...
const RATE_CARD_FIELDS: RateCardField[] = ["media_outlet", "property", "format", "price", "currency", "pricing_unit", "valid_from", "valid_to", "source", "source_reference", "notes"];
const FIELD_LABEL_KEYS: Record<RateCardField, string> = {
  media_outlet: "media.field.mediaOutlet", property: "media.field.property", format: "media.formatLabel",
  price: "media.priceLabel", currency: "media.currencyLabel", pricing_unit: "media.pricingUnitLabel",
  valid_from: "media.validFromLabel", valid_to: "media.field.validTo", source: "media.field.source",
...
export function RateCardImportFlow({
  knownPlatforms, knownFormats,
}: {
  ...
  const [mappings, setMappings] = useState<RateCardColumnMapping[]>([]);
  const [validatedRows, setValidatedRows] = useState<ValidatedRateCardRow[]>([]);
  ...
  const formatByKey = new Map(knownFormats.map((f) => [f.internal_key, f]));
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    setMappings(detectRateCardMapping(parsed.table));
    setStep("columns");
  }, [t]);
  ...
  function updateMapping(columnIndex: number, field: RateCardField | "ignore") {
  ...
  async function confirmImport() {
    ...
    const payload = validatedRows
      ...
      .map((r) => ({
        ...
        price: r.price!,
        currency: r.currency!,
        pricingUnit: r.pricingUnit as "per_integration" | "per_spot" | "per_mention" | "per_day" | "per_week" | "per_month" | "per_thousand" | "package" | "custom",
    ...
    const res = await bulkSubmitRateCardsAction(payload);
    setSubmitting(false);
      ...
      <div className="md:pl-[var(--sidebar-inset)] transition-[padding-left] duration-150">
        <main className="mx-auto max-w-4xl px-4 py-6 md:px-8">
          ...
          <div className="flex items-center gap-2">
            <Tag size={18} className="text-brandPeach" aria-hidden="true" />
            <h1 className="font-display text-xl font-semibold text-ink-900">{t("media.importRateCardsTitle")}</h1>
          </div>
          ...
          {step === "file" && (
            <div className="mt-3">
              <button onClick={() => triggerDownload(new Blob([generateRateCardCsvTemplate()], { type: "text/csv" }), "cucurucho-tarifarios.csv")} className="mr-2 inline-flex items-center gap-1.5 rounded-full border border-line px-3 py-1.5 text-xs font-medium text-ink-700 hover:border-primary hover:text-primary">
                ...
                {mappings.map((m) => (
                  <div key={m.sourceColumnIndex} className="flex flex-wrap items-center gap-3 rounded-xl border border-line bg-surface px-3 py-2">
                    ...
                    <select
                      ...
                      onChange={(e) => updateMapping(m.sourceColumnIndex, e.target.value === "ignore" ? "ignore" : e.target.value as RateCardField)}
                    ...
                    >
                      ...
                      {RATE_CARD_FIELDS.map((f) => <option key={f} value={f}>{t(FIELD_LABEL_KEYS[f])}</option>)}
                    </select>
                ...
                <table className="w-full min-w-[560px] text-left text-xs">
                  <thead className="sticky top-0 bg-surface2 text-ink-500">
                    <tr>
                      ...
                      <th scope="col" className="px-3 py-2">{t("media.formatLabel")}</th>
                      <th scope="col" className="px-3 py-2">{t("media.priceLabel")}</th>
